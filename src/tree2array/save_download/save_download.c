...
int main(){
	...
	*pom_i=k_y;
	...
	day_adr=(int*)(mas+(k_y+1)*sizeof(int)+0*31*12*sizeof(int)+3*31*sizeof(int)+5*sizeof(int));
	pom_i=(int*)(&mas[*day_adr]);
	pom_d=(double*)(&mas[*day_adr]+sizeof(int));
	pom_d1=(double*)(&mas[*day_adr]+sizeof(int)+sizeof(double));
	printf("%d %lf %lf\n",*pom_i,*pom_d,*pom_d1);
									//меню запросов
	make_menu(1);
	printf("\n Выберите команду - ");
	while(scanf("%d", &g)==1)
	{
		...
		if(g==5)
		{
			...
			//zap_vse(mas, mas_years, year, k_coor);
		}
		fclose(zap);
		printf("\n Выберите команду - ");
	}
...
void zap_day(char *mas, int *mas_years, int year, int month, int day, int k_coor)
{
	...
	t=0;
	while( (&mas[*day_adr] + t*sizeof(int) + 2*t*sizeof(double))!=&mas[*day_adr_sled] && (&mas[*day_adr] + t*sizeof(int) + 2*t*sizeof(double))!=&mas[(k_y+1)*sizeof(int)+12*31*k_y*sizeof(int)+(k_coor-1)*(sizeof(int)+2*sizeof(double))])
	{
